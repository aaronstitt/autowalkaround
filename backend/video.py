...
async def generate_video(
    ...
    # Verify salesperson exists and belongs to this dealership
    ...
    # Validate salesperson has HeyGen IDs set
...
async def run_pipeline(job_id: str, vehicle_url: str, salesperson: dict, dealership_id: str):
        """
            Full async pipeline:
                1. Scrape vehicle page (photos, features, details)
                    2. Generate script via GPT-4o-mini
                        3. Submit to HeyGen API, poll until complete
                            4. Composite avatar over animated vehicle photos with FFmpeg
                                5. Upload to Supabase Storage
                                    6. Update job record to completed
                                        """
    ...
    try:
                ...
                # Run sync scraper in thread pool to not block event loop
        ...
        # Pick vehicle photos for HeyGen backgrounds
...
def upload_to_storage(file_path: str, job_id: str) -> str | None:
        """Upload final video to Supabase Storage. Returns public URL or None on failure."""
    try:
        ...
        # Try to upload (upsert in case of retry)
...
async def download_video(job_id: str, current_user: dict = Depends(get_current_user)):
    ...
    # Prefer Supabase Storage URL (persists across Railway restarts)
    if j.get('output_url'):
                return RedirectResponse(url=j['output_url'])

    # Fallback to local file (Railway ephemeral storage - may not exist on restart)
    ...
    raise HTTPException(status_code=404, detail='Video file not found. It may have been cleared from server storage. Regenerate the video.')
