...
# Poll every 15s, up to 300 polls = 75 minutes max
...
def generate_heygen_webm(script_text, avatar_look_id, voice_id, tmpdir):
    ...
    print(f'[HeyGen] Video ID: {video_id} - polling every {HEYGEN_POLL_INTERVAL}s (max {HEYGEN_POLL_MAX} polls = {HEYGEN_POLL_MAX * HEYGEN_POLL_INTERVAL // 60} min)...')
...
def composite_avatar_over_background(webm_path, bg_image_path, audio_start, duration, output_mp4, tmpdir):
    ...
    overlay_filter = f'[1:v]scale={aaron_w}:{aaron_h}[avatar];[0:v][avatar]overlay={aaron_x}:{aaron_y}:format=auto[out]'
