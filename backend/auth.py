...
def create_token(user_id: str, email: str, dealership_id: str) -> str:
    payload = {'sub': user_id, 'email': email, 'dealership_id': dealership_id, 'exp': datetime.utcnow() + timedelta(hours=JWT_EXPIRE_HOURS)}
...
async def signup(req: SignupRequest):
    ...
    dealership = supabase.table('dealerships').insert({'name': req.dealership_name, 'website_url': req.website_url, 'plan': 'free', 'videos_generated_this_month': 0}).execute()
    dealership_id = dealership.data[0]['id']
    hashed_pw = hash_password(req.password)
    user = supabase.table('users').insert({'email': req.email, 'password_hash': hashed_pw, 'dealership_id': dealership_id, 'contact_name': req.contact_name, 'role': 'admin'}).execute()
